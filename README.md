...
As for the scripts, the first is titled "Inference.py" and is able to take a folder input and produce the resulting inferences from the two highest performing models in either object class: PBW7 and CW7. It mostly contains a function labeled "inferences" which takes two arguments. The first is the path to the image folder while the second is a ture or false value as to if the user wants the inferences to be displayed while the model makes predictions.
